...
type InlineChoiceOpt = { id?: string; value?: string; label?: string; text?: string };
type InlineChoiceBlank = { id?: string; blankId?: string; options?: InlineChoiceOpt[]; choices?: InlineChoiceOpt[] };
...
export default function InlineChoice({
  ...
  const text: string = String(item?.clozeText ?? item?.prompt ?? item?.stem ?? "");
      ...
      <div className="text-base leading-relaxed">
        {parts.map((p, i) =>
          p.t === "text" ? (
            ...
            <span key={i} className="mx-1 inline-flex flex-wrap gap-2 align-baseline">
              {optsFor(p.v).map((o, j) => {
