...
echo "== Patch: reset Teacher Dashboard page.tsx to clean JSX =="
...
TARGET="apps/web/src/app/(app)/teacher/dashboard/page.tsx"
if [[ ! -f "$TARGET" ]]; then
  # Try to locate if path differs
  FOUND="$(git ls-files | grep -E 'teacher/dashboard/page\.tsx$' | head -n 1 || true)"
  if [[ -z "${FOUND}" ]]; then
...
echo "Reset complete."
echo "Now run your dev/build again."
