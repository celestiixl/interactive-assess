      ...
      <div className="mx-auto max-w-7xl px-4">
        <div className="flex flex-wrap items-center justify-between gap-3 py-3">
          <div className="min-w-0">
            ...
            {metaLeft ? <div className="text-xs text-slate-500">{metaLeft}</div> : null}
          </div>

          <div className="flex items-center gap-2">
            <ModeToggle mode={mode} onChange={onModeChange} />
            <ToolButton label="Notes" onClick={onOpenNotes} />
            <ToolButton label={flagged ? "Flagged" : "Flag"} onClick={onToggleFlag} active={!!flagged} />
          </div>
        </div>
      </div>
    </div>
    ...
    <div className="flex rounded-xl border border-slate-200 bg-white p-1">
      <button
        ...
        className={[
          ...
          mode === "learn" ? "bg-slate-100 text-slate-900" : "text-slate-600 hover:bg-slate-50",
        ].join(" ")}
      >
        Learn
      </button>
      <button
        ...
        className={[
          "rounded-lg px-3 py-1.5 text-xs",
          mode === "exam" ? "bg-slate-100 text-slate-900" : "text-slate-600 hover:bg-slate-50",
        ].join(" ")}
      >
        Exam
      </button>
