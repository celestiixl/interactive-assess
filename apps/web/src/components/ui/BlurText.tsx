...
const buildKeyframes = (
  ...
  const keys = new Set([...Object.keys(from), ...steps.flatMap(s => Object.keys(s))]) as Set<keyof AnimationSnapshot>;
  const keyframes: Record<string, (string | number)[]> = {};
  keys.forEach(k => {
    const fromVal = from[k] ?? (k === 'filter' ? 'blur(0px)' : k === 'opacity' ? 1 : 0);
    keyframes[k as string] = [fromVal as string | number, ...steps.map(s => (s[k] ?? (k === 'filter' ? 'blur(0px)' : k === 'opacity' ? 1 : 0)) as string | number)];
  });
  return keyframes;
};
