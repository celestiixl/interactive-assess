...
const [attemptsById, setAttemptsById] = useState<Record<string, number>>({});
  ...
  const [lastResult, setLastResult] = useState<null | { score: number; max: number; correct: boolean; timeSeconds: number; attemptsLeft: number; canShowExplanation: boolean }>(null);
...
useEffect(() => {
  ...
  return (
    <AppShell activeKey="practice"><div className="space-y-6 h-dvh w-full overflow-y-auto bg-slate-50 text-[15px] leading-normal">
      ...
      <div className="mx-auto max-w-7xl px-4 py-5 pb-10 space-y-6">
        <div className="flex items-center justify-between">
          <h1 className="text-2xl font-bold">Practice: {rcParam || rcLabels[0]}</h1>
        ...
        <div className="flex flex-wrap gap-2">
          {rcLabels.map((rc) => (
            <button
              key={rc}
              onClick={() => router.replace(`/practice?rc=${encodeURIComponent(rc)}`)}
            ...
            left={
              <div className="space-y-3">
                ...
                <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
                  ...
                  <BilingualText
                      text={(safeItem as any)?.prompt ?? (safeItem as any)?.stem ?? "No prompt found for this item yet."}
...
/>
            ...
            right={
              <div className="space-y-3">
                ...
                {!safeItem ? (
                  <div className="p-4 border rounded bg-neutral-50">Couldn&apos;t load this item. Try another one.</div>
                ) : (
                  <ItemRenderer
  ...
  mode === "learn" && (correct || allowed === Infinity || usedNow >= allowed);
                  ...
                  <button
                      ...
                      onClick={() => {
                        if (!checkedThisItem) {
                          ...
                          const btn = document.querySelector('button[data-check-button="true"]') as HTMLButtonElement | null;
                          if (btn) {
                            btn.click();
                            return;
                          }
                      ...
                      disabled={(!safeItem) || (checkedThisItem && safeIndex === mergedItems.length - 1)}
                    >
                      {checkedThisItem ? (safeIndex === mergedItems.length - 1 ? "Done" : "Continue") : "Check Answer"}
                    </button>
                </div>
              </div>
            }
          ...
          open={drawerOpen}
            ...
            explanation={
              lastResult?.canShowExplanation && mode === "learn"
                ? ((safeItem as any)?.rationale ?? "Explanation goes here (wire to item later).")
                : undefined
            }
