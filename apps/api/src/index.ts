...
app.get('/redo', async (req) => {
  ...
  const redoItems = [...byItem.entries()]
    .filter(([_, arr]) => arr[arr.length - 1].score < arr[arr.length - 1].maxScore) // latest wrong
...
app.post('/assignment/progress', async (req, reply) => {
  ...
  const baseRequired = Number.isFinite(body.baseRequired) ? Number(body.baseRequired) : 5; // C
  const penaltyPerWrong = Number.isFinite(body.penaltyPerWrong) ? Number(body.penaltyPerWrong) : 1; // P

  // latest per item
  const latestByItem = new Map<string, { score: number; max: number; ts: number }>();
  for (const a of attempts) {
    if (a.userId !== userId) continue;
    if (!ids.includes(a.itemId)) continue;
    const prev = latestByItem.get(a.itemId);
    if (!prev || a.ts > prev.ts) latestByItem.set(a.itemId, { score: a.score, max: a.maxScore, ts: a.ts });
  }
