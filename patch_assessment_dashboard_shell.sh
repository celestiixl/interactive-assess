...
cp -v "$PAGE" "$PAGE.bak.$(date +%Y%m%d_%H%M%S)" >/dev/null || true
...
function Pill({
  ...
  const tones: Record<string, string> = {
    neutral: "border-foreground/10 bg-background text-foreground/70",
    teal: "border-teal-600/30 bg-teal-500/10 text-teal-900 dark:text-teal-200",
    emerald:
      "border-emerald-600/30 bg-emerald-500/10 text-emerald-900 dark:text-emerald-200",
    amber:
      "border-amber-600/30 bg-amber-500/10 text-amber-950 dark:text-amber-200",
    violet:
      "border-violet-600/30 bg-violet-500/10 text-violet-950 dark:text-violet-200",
    slate:
      "border-slate-600/25 bg-slate-500/10 text-slate-900 dark:text-slate-200",
  };
...
function ActionCard({
  ...
  return (
    <Link
      href={href}
      className="group relative overflow-hidden rounded-2xl border border-foreground/10 bg-background p-6 shadow-[0_14px_40px_rgba(2,6,23,0.10)] transition hover:-translate-y-0.5 hover:shadow-[0_18px_55px_rgba(2,6,23,0.14)]"
    >
      ...
      <div className="flex items-start justify-between gap-6">
        ...
        <div className="inline-flex h-11 w-11 items-center justify-center rounded-2xl border border-foreground/10 bg-muted/20 text-lg text-foreground/70 transition group-hover:bg-muted/35 group-hover:text-foreground">
          →
        </div>
      </div>
...
export default function AssessmentDashboardEntry() {
  return (
    <div className="min-h-screen bg-gradient-to-b from-slate-50 via-white to-slate-50 dark:from-slate-950 dark:via-black dark:to-slate-950">
      {/* Top bar */}
      <header className="sticky top-0 z-20 border-b border-foreground/10 bg-background/80 backdrop-blur">
        <div className="mx-auto flex w-full max-w-screen-2xl items-center justify-between gap-4 px-6 py-4">
          <div className="flex items-center gap-3">
            <div className="grid h-10 w-10 place-items-center rounded-2xl border border-foreground/10 bg-muted/30 text-sm font-bold">
              IA
            </div>
            <div>
          ...
          <div className="flex items-center gap-2">
            <Link
              href="/practice"
              className="rounded-xl border border-foreground/10 bg-muted/20 px-4 py-2 text-sm font-semibold transition hover:bg-muted/35"
            >
              Open Practice
            </Link>
            <Link
      ...
      <div className="mx-auto grid w-full max-w-screen-2xl grid-cols-1 gap-6 px-6 py-8 lg:grid-cols-[280px_1fr]">
        {/* Sidebar */}
        <aside className="rounded-2xl border border-foreground/10 bg-background p-4 shadow-[0_14px_40px_rgba(2,6,23,0.08)]">
          <div className="mb-4">
          ...
          <div className="mt-5 rounded-2xl border border-foreground/10 bg-muted/15 p-4">
            ...
            <div className="mt-1 text-sm">
              ...
              <span className="text-foreground/70">Inline Choice first-class</span>
            </div>
          </div>
        </aside>
        ...
        <main className="space-y-6">
          ...
          <div className="grid grid-cols-1 gap-4 xl:grid-cols-3">
            <div className="xl:col-span-2 rounded-2xl border border-foreground/10 bg-background p-6 shadow-[0_14px_40px_rgba(2,6,23,0.08)]">
            ...
            <div className="rounded-2xl border border-foreground/10 bg-background p-6 shadow-[0_14px_40px_rgba(2,6,23,0.08)]">
              <div className="flex items-center justify-between">
...
echo "✅ /assessment now uses a dashboard shell layout"
